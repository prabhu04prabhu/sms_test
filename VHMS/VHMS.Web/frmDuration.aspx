<%@ Page Title="Duration" Language="C#" MasterPageFile="~/VHMSMasterPage.master" AutoEventWireup="true" CodeFile="frmDuration.aspx.cs" Inherits="frmDuration" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="VHMSWebContent" runat="Server">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="form-group" id="divName">
                                ...
                                <input type="text" class="form-control" id="txtName" placeholder="Please enter Duration Name"
    ...
    <script type="text/javascript">
        ...
        $("#btnAddNew").click(function () {
            ...
            $("#btnSave").show();
            $("#btnUpdate").hide();
            $(".modal-title").html("<i class='fa fa-plus-square'></i>&nbsp;&nbsp;Add Duration");
        ...
        $("#btnSave,#btnUpdate").click(function () {
            ...
            var Obj = new Object();
            Obj.DurationID = 0;
            Obj.DurationName = $("#txtName").val().trim();
            Obj.DurationCode = $("#txtCode").val();
            Obj.IsActive = $("#chkStatus").is(':checked') ? "1" : "0";

            var sMethodName;
            if ($("#hdnID").val() > 0) {
                Obj.DurationID = $("#hdnID").val();
                sMethodName = "UpdateDuration";
            }
            else { sMethodName = "AddDuration"; }

            SaveandUpdateDuration(Obj, sMethodName);
            return false;
        });
                            ...
                            if (objResponse.Value != null && objResponse.Value != "NoRecord") {
                                ...
                                if (obj.length > 0) {
                                    ...
                                    for (var index = 0; index < obj.length; index++) {
                                        ...
                                        var table = "<tr id='" + obj[index].DurationID + "'>";
                                        table += "<td>" + (index + 1) + "</td>";
                                        table += "<td>" + obj[index].DurationName + "</td>";
                                        table += "<td class='hidden-xs'>" + obj[index].DurationCode + "</td>";
                                        table += "<td class='hidden-xs'>" + TypeStatus + "</td>";

                                        if (ActionView == "1")
                                        { table += "<td style='text-align:center;'><a href='#' id=" + obj[index].DurationID + " class='View' title='Click here to View'><i class='fa fa-lg fa-clone text-yellow'/></a></td>"; }
                                        else { table += "<td></td>"; }

                                        if (ActionUpdate == "1")
                                        { table += "<td style='text-align:center;'><a href='#' id=" + obj[index].DurationID + " class='Edit' title='Click here to Edit'><i class='fa fa-lg fa-edit'/></a></td>"; }
                                        else
                                        { table += "<td></td>"; }

                                        if (ActionDelete == "1")
                                        { table += "<td style='text-align:center;'><a href='#' id=" + obj[index].DurationID + " class='Delete' title='Click here to Delete'><i class='fa fa-lg fa-trash-o text-red'/></a></td>"; }
                                        else
                                        { table += "<td></td>"; }
                                    ...
                                    $(".View").click(function () {
                                        if (ActionView == "1") {
                                            ...
                                            $(".modal-title").html("<i class='fa fa-clone'></i>&nbsp;&nbsp;View Duration");
                                            $("#btnSave").hide();
                                            $("#btnUpdate").hide();
                                        }
                                        else {
        ...
        function SaveandUpdateDuration(Obj, sMethodName) {
            $.ajax({
                type: "POST",
                ...
                success: function (data) {
                    if (data.d != "") {
                        ...
                        if (objResponse.Status == "Success") {
                            ...
                            else if (objResponse.Value == "Duration_A_01" || objResponse.Value == "Duration_U_01") {
        ...
        function EditRecord(id) {
            ...
            $.ajax({
                type: "POST",
                url: "WebServices/VHMSService.svc/GetDurationByID",
                        ...
                        if (objResponse.Status == "Success") {
                            if (objResponse.Value != null && objResponse.Value != "NoRecord" && objResponse.Value != "Error") {
                                ...
                                if (obj != null) {
                                    ...
                                    $("#hdnID").val(obj.DurationID);
                                    $("#txtName").val(obj.DurationName);
                                    $("#txtCode").val(obj.DurationCode);

                                    $("#chkStatus").prop("checked", obj.IsActive ? true : false);

                                    $('#compose-modal').modal({ show: true, backdrop: true });
                                    $(".modal-title").html("<i class='fa fa-pencil'></i>&nbsp;&nbsp;Edit Duration");
                                }
                                dProgress(false);
                            }
                            else if (objResponse.Value == "NoRecord") {
        ...
        function DeleteRecord(id) {
            $.ajax({
                type: "POST",
                url: "WebServices/VHMSService.svc/DeleteDuration",
                data: JSON.stringify({ ID: id }),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                async: false,
                success: function (data) {
                    if (data.d != "") {
                        ...
                        if (objResponse.Status == "Success") {
                            ...
                            else if (objResponse.Value == "Duration_R_01" || objResponse.Value == "Duration_D_01") {
