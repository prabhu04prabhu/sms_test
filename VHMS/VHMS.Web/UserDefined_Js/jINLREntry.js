...
$(function () {
    ...
    if (PurchaseID != undefined && PurchaseID.length > 0) {
        GetINLREntrySalesID(PurchaseID)
...
function GetINLREntrySalesID(PurchaseID) {
    dProgress(true);
    $.ajax({
        type: "POST",
        url: "WebServices/VHMSService.svc/GetINLREntrySalesID",
                ...
                if (objResponse.Status == "Success") {
                    if (objResponse.Value != null && objResponse.Value != "NoRecord" && objResponse.Value != "Error") {
                        ...
                        if (obj != null) {
                            ...
                            $("#hdnPurchaseID").val(parseInt(obj.INLREntryID));
                        }
                        dProgress(false);
                    }
...
function GetSalesDetails(id) {
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/VHMSService.svc/GetPurchaseInvoice",
            ...
            if (data.d != "") {
                ...
                if (objResponse.Status == "Success") {
                    if (objResponse.Value != null && objResponse.Value != "NoRecord" && objResponse.Value != "Error") {
                        var obj = jQuery.parseJSON(objResponse.Value);
                        if (obj != null) {
                            $("#ddlCustomerName").val(obj.Supplier.SupplierID).change();
...
$("#btnAddMagazine,#btnUpdateMagazine").click(function () {
    ...
    if (this.id == "btnAddMagazine") {
        ...
        if ($("#hdnINLREntryID").val() > 0) {
            ObjData.StatusFlag = "U";
            ObjData.INLREntryID = $("#hdnINLREntryID").val();
        }
        else {
            ObjData.StatusFlag = "I";
            ObjData.INLREntryID = 0;
...
function Bind_OPBillingByID(ID, data) {
    ...
    for (var i = 0; i < data.length; i++) {
        if (data[i].sNO == ID) {
            $("#hdnOPSNo").val(ID);
            $("#hdnINLREntryID").val(data[i].INLREntryID);
...
function Update_OPBilling(oData) {
    for (var i = 0; i < gOPBillingList.length; i++) {
        if (gOPBillingList[i].sNO == oData.sNO) {
            gOPBillingList[i].INLREntryID = oData.INLREntryID;
...
function GetRecord() {
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/VHMSService.svc/GetINLREntry",
                    ...
                    if (objResponse.Value != null && objResponse.Value != "NoRecord") {
                        ...
                        if (obj.length > 0) {
                            ...
                            for (var index = 0; index < obj.length; index++) {
                                ...
                                var table = "<tr id='" + obj[index].INLREntryID + "'>";
                                table += "<td>" + (index + 1) + "</td>";
                                ...
                                if (ActionView == "1") { table += "<td style='text-align:center;'><a href='#' id=" + obj[index].INLREntryID + " class='View' title='Click here to View'><i class='fa fa-lg fa-clone text-yellow'/></a></td>"; }
                                else { table += "<td></td>"; }

                                if (ActionUpdate == "1") { table += "<td style='text-align:center;'><a href='#' id=" + obj[index].INLREntryID + " class='Edit' title='Click here to Edit'><i class='fa fa-lg fa-edit'/></a></td>"; }
                                else { table += "<td></td>"; }

                                if (ActionDelete == "1") { table += "<td style='text-align:center;'><a href='#' id=" + obj[index].INLREntryID + " class='Delete' title='Click here to Cancel'><i class='fa fa-lg fa-times-circle text-red'/></a></td>"; }
                                else { table += "<td></td>"; }

                                table += "<td style='text-align:center;'><a href='#' id=" + obj[index].INLREntryID + " class='WhatsApp' title='Click here to send WhatsApp SMS'><i class='fa fa-lg fa-whatsapp text-green'/></a></td>";
                                table += "</tr>";
...
function GetSearchRecord(iDetails) {
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/VHMSService.svc/SearchINLREntry",
                    ...
                    if (objResponse.Value != null && objResponse.Value != "NoRecord") {
                        ...
                        if (obj.length > 0) {
                            ...
                            for (var index = 0; index < obj.length; index++) {
                                ...
                                table += "<td>" + (index + 1) + "</td>";
                                table += "<td>" + obj[index].LREntryNo + "</td>";
                                table += "<td>" + obj[index].sINLREntryDate + "</td>";
                                table += "<td>" + obj[index].Supplier.SupplierName + "</td>";
                                table += "<td>" + obj[index].Transport.TransportName + "</td>";
                                table += "<td>" + obj[index].PurchaseNo + "</td>";
                                table += "<td>" + obj[index].Status + "</td>";
...
$("#btnSave,#btnUpdate").click(function () {
    ...
    objSupplier.SupplierID = $("#ddlCustomerName").val();
    ...
    ObjOPBilling.DocumentPath = $("[id*=imgUpload2_view]").attr("src");
    ObjOPBilling.DocumentPath1 = $("[id*=imgUpload3_view]").attr("src");

    ObjOPBilling.INLREntryID = 0;
    ...
    ObjOPBilling.INLREntryTrans = gOPBillingList;
    ...
    if ($("#hdnINLREntryID").val() > 0) {
        ObjOPBilling.INLREntryID = $("#hdnINLREntryID").val();
        gOPBillingList.INLREntryID = ObjOPBilling.INLREntryID;
        ObjOPBilling.OPBillingTrans = gOPBillingList;
        sMethodName = "UpdateINLREntry";
    }
    else {
        sMethodName = "AddINLREntry";
        ObjOPBilling.INLREntryID = 0
    }
                ...
                if (objResponse.Status == "Success") {
                    if (objResponse.Value > 0) {
                        ...
                        if (sMethodName == "AddOPBilling") {
                            ...
                            $("#hdnINLREntryID").val(objResponse.Value);
                        }
...
function EditRecord(id) {
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/VHMSService.svc/GetINLREntryByID",
        data: JSON.stringify({ ID: id }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
                ...
                if (objResponse.Status == "Success") {
                    if (objResponse.Value != null && objResponse.Value != "NoRecord" && objResponse.Value != "Error") {
                        ...
                        if (obj != null) {
                            ...
                            $("#txtInvoiceNo").val(obj.PurchaseNo).change();
                            $("#hdnINLREntryID").val(obj.INLREntryID)
                           ...
                           // $("#ddlShippingAddress").val(obj.ShippingAddress.ShippingAddressID).change();
                            ...
                            $("[id*=imgUpload2_view]").attr("src", obj.DocumentPath);
                            $("[id*=imgUpload3_view]").css("visibility", "visible");
                            $("[id*=imgUpload3_view]").attr("src", obj.DocumentPath1);

                            gOPBillingList = [];
                            var ObjProduct = obj.INLREntryTrans;
                            for (var index = 0; index < ObjProduct.length; index++) {
                                ...
                                objTemp.INLREntryTransID = ObjProduct[index].INLREntryTransID;
                                objTemp.INLREntryID = ObjProduct[index].INLREntryID;
...
function DeleteRecord(id, Reason) {
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/VHMSService.svc/DeleteINLREntry",
                    ...
                    if (objResponse.Value != null && objResponse.Value != "NoRecord" && objResponse.Value != "Error") {
                        ...
                        if (obj != null) {
                            ...
                            if (Message.match(/#InvoiceNo#/))
                                Message = Message.replace("#InvoiceNo#", obj.PurchaseNo ) ;
