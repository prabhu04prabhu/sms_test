...
$("#ddlCustomerName").change(function () {
    if ($("#ddlCustomerName").val() > 0) {
        GetCustomerByID();
        GetoldSalesInvoiceNo();
    }
});

function GetoldSalesInvoiceNo() {
    var sControlName = "#ddlOldSalesInvoice";
    dProgress(true);
    $(sControlName).empty();
    $.ajax({
        type: "POST",
        url: "WebServices/VHMSService.svc/GetTDSSalesEntry",
        data: JSON.stringify({ PublisherID: $("#ddlCustomerName").val(), TDSSalesID: $("#hdnTDSPaymentID").val(), IsRetail: 0 }),
        contentType: "application/json; charset=utf-8",
        async: false,
        success: function (data) {
            if (data.d != "") {
                        ...
                        if (obj.length > 0) {
                            ...
                            for (var index = 0; index < obj.length; index++) {
                                if (obj[index].IsActive)
                                    $(sControlName).append("<option value='" + obj[index].SalesEntryID + "'>" + obj[index].InvoiceNo + " - " + (obj[index].sInvoiceDate) + "</option>");
                            }
...
function GetCustomerByID() {
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/VHMSService.svc/GetAdjustTDSSalesEntry",
        data: JSON.stringify({ PublisherID: $("#ddlCustomerName").val(), TDSSalesID: $("#hdnTDSPaymentID").val(), IsRetail: 0 }),
        contentType: "application/json; charset=utf-8",
        async: false,
        success: function (data) {
...
function GetRate() {
    if ($("#ddlProductName").val() > 0) {
        ...
        $.ajax({
            type: "POST",
            url: "WebServices/VHMSService.svc/GetTDSSalesEntryByID",
            data: JSON.stringify({ ID: $("#ddlProductName").val(), IsRetail: 0 }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
            success: function (data) {
...
function GetOldSalesInvoiceRate() {
    if ($("#ddlOldSalesInvoice").val() > 0) {
        ...
        $.ajax({
            type: "POST",
            url: "WebServices/VHMSService.svc/GetTDSSalesEntryByID",
            data: JSON.stringify({ ID: $("#ddlOldSalesInvoice").val(), IsRetail: 0 }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
            success: function (data) {
...
$("#btnAddMagazine,#btnUpdateMagazine").click(function () {
    ...
    oProduct.SalesEntryID = $("#ddlProductName").val();
    oProduct.InvoiceNo = $("#ddlProductName option:selected").text();
    oProduct.NetAmount = parseFloat($("#txtAvailableQty").val());
    ObjData.SalesEntry = oProduct;

    var OldSales = new Object();
    OldSales.SalesEntryID = $("#ddlOldSalesInvoice").val();
    OldSales.InvoiceNo = $("#ddlOldSalesInvoice option:selected").text();
    OldSales.NetAmount = parseFloat($("#txtOldSalesTotalAmt").val());
    ObjData.oldSalesEntry = OldSales;
...
function DisplayTDSPaymentList(gData) {
    ...
    if (gData.length > 0) {
        ...
        sTable += "<th class='" + sColorCode + "'>Old Sales Invoice No</th>";
        sTable += "<th class='" + sColorCode + "'>Old Sales TotalAmount</th>";
        sTable += "<th class='" + sColorCode + "'>Adjust Sales Invoice No</th>";
        sTable += "<th class='" + sColorCode + "'>Adjust Sales TotalAmount</th>";
        ...
        for (var i = 0; i < gData.length; i++) {
            if (gData[i].StatusFlag != "D") {
                ...
                $("#txtSNo").val(sCount + 1);
                sTable += "<td>" + gData[i].oldSalesEntry.InvoiceNo + "</td>";
                sTable += "<td>" + gData[i].oldSalesEntry.NetAmount + "</td>";
                sTable += "<td>" + gData[i].SalesEntry.InvoiceNo + "</td>";
                sTable += "<td>" + gData[i].SalesEntry.NetAmount + "</td>";
    ...
    for (var i = 0; i < data.length; i++) {
        if (data[i].sNO == ID) {
            if (data[i].StatusFlag != "D") {
                ...
                $("#ddlProductName").val(data[i].SalesEntry.SalesEntryID).change();
                $("#ddlOldSalesInvoice").val(data[i].oldSalesEntry.SalesEntryID).change();
...
function Update_TDSPayment(oData) {
    for (var i = 0; i < gOPBillingList.length; i++) {
        if (gOPBillingList[i].sNO == oData.sNO) {
            ...
            var oProduct = new Object();
            oProduct.SalesEntryID = oData.SalesEntry.SalesEntryID;
            oProduct.InvoiceNo = oData.SalesEntry.InvoiceNo;
            oProduct.NetAmount = oData.SalesEntry.NetAmount;
            gOPBillingList[i].SalesEntry = oProduct;

            var OldSales = new Object();
            OldSales.SalesEntryID = oData.oldSalesEntry.SalesEntryID;
            OldSales.InvoiceNo = oData.oldSalesEntry.InvoiceNo;
            OldSales.NetAmount = oData.oldSalesEntry.NetAmount;
            gOPBillingList[i].oldSalesEntry = OldSales;
...
function GetRecord() {
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/VHMSService.svc/GetTDSPayment",
...
function GetSearchRecord(iDetails) {
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/VHMSService.svc/SearchTDSPayment",
...
$("#btnSave,#btnUpdate").click(function () {
    ...
    ObjTDSPayment.ManualPayment=$("#ddlManualPayment").val(); 
    ...
    ObjTDSPayment.TDSPaymentTrans = gOPBillingList;
    ...
    if ($("#hdnTDSPaymentID").val() > 0) {
        ...
        sMethodName = "UpdateTDSPayment";
    }
    else {
        sMethodName = "AddTDSPayment";
        ObjTDSPayment.TDSPaymentID = 0
    }
            ...
            if (data.d != "") {
                ...
                if (objResponse.Status == "Success") {
                    if (objResponse.Value > 0) {
                        ...
                        else if (sMethodName == "UpdateTDSPayment") { $.jGrowl("Updated Successfully", { sticky: false, theme: 'success', life: jGrowlLife }); }
...
function EditRecord(id) {
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/VHMSService.svc/GetTDSPaymentByID",
                ...
                if (objResponse.Status == "Success") {
                    if (objResponse.Value != null && objResponse.Value != "NoRecord" && objResponse.Value != "Error") {
                        ...
                        if (obj != null) {
                            ...
                            $("#ddlManualPayment").val(obj.ManualPayment).change(); 
                            ...
                            gOPBillingList = [];
                            var ObjProduct = obj.TDSPaymentTrans;
                            for (var index = 0; index < ObjProduct.length; index++) {
                                ...
                                objProduct.SalesEntryID = ObjProduct[index].SalesEntry.SalesEntryID;
                                objProduct.InvoiceNo = ObjProduct[index].SalesEntry.InvoiceNo;
                                objProduct.NetAmount = ObjProduct[index].SalesEntry.NetAmount;
                                objTemp.SalesEntry = objProduct;

                                var OldSalesInvoice = new Object();
                                OldSalesInvoice.SalesEntryID = ObjProduct[index].oldSalesEntry.SalesEntryID;
                                OldSalesInvoice.InvoiceNo = ObjProduct[index].oldSalesEntry.InvoiceNo;
                                OldSalesInvoice.NetAmount = ObjProduct[index].oldSalesEntry.NetAmount;
                                objTemp.oldSalesEntry = OldSalesInvoice;
...
function DeleteRecord(id, Reason) {
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/VHMSService.svc/DeleteTDSPayment",
        data: JSON.stringify({ ID: id }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
