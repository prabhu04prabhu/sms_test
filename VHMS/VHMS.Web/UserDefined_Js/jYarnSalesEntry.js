...
function GetProductDetails(id, smscode, value, SID, SalesID) {
    dProgress(true);
    $.ajax({
        type: "POST",
...
function ClearOPBillingTab() {
    ...
    $("#ddlProductName").attr("disabled", false);
...
$("#txtCode").blur(function () {
    ...
    if ($("#txtCode").val().length > 2) {
        ...
        $("#composedetails").modal('hide');
        $("#Allcomposedetails").modal('hide');
        $("#Yarncomposedetails").modal({ show: true, backdrop: true });
        GetYarnProductDetails($("#ddlProductName").val());
    }
...
function GetYarnProductDetails(id) {
    $("#hdnBarcodeID").val("0");
    $("#divYarnDetailsList").empty();
    $(".Yarnmodal-title").html("");
    gvalue = [];
    dProgress(true);
    $.ajax({
        type: "POST",
        url: "WebServices/VHMSService.svc/BarcodeYarnStock",
                ...
                if (objResponse.Status == "Success") {
                    if (objResponse.Value != null && objResponse.Value != "NoRecord" && objResponse.Value != "Error") {
                        ...
                        if (obj != null) {
                            ...
                            if (obj.length >= 5) { $("#divYarnDetailsList").css({ 'height': '0px', 'min-height': '200px', 'overflow': 'auto' }); }
                            else { $("#divYarnDetailsList").css({ 'height': '', 'min-height': '' }); }

                            if (obj.length > 0) {
                                sTable = "<table id='tblYarnDetailsList' class='table no-margin table-condensed table-hover'>";
                                sTable += "<thead><tr><th class='" + sColorCode + "' style='width:3px;text-align: center'>S.No</th>";
                                sTable += "<th style='line-height:0.5;' class='" + sColorCode + "'>Barcode</th>";
                                sTable += "<th style='line-height:0.5;' class='" + sColorCode + "'>Quantity</th>";
                                sTable += "<th style='line-height:0.5;' class='" + sColorCode + "'>PurchasePrice</th>";
                                sTable += "</tr></thead><tbody id='tblYarnDetailsList_body'>";
                                sTable += "</tbody></table>";
                                $("#divYarnDetailsList").html(sTable);
                                for (var i = 0; i < obj.length; i++) {
                                    sTable = "<tr><td id='" + obj[i].BarcodeID + "'>" + sCount + "</td>";
                                    //sTable += "<td>" + obj[i].Barcodes + "</td>";
                                    sTable += "<td style='line-height:0.5;'><label><input  id='chk_" + obj[i].BarcodeID + "' type='radio' value='" + obj[i].BarcodeID + "'  name='PurchaseName'/>&nbsp;" + obj[i].Barcodes + "</label></td>";
                                    sTable += "<td style='line-height:0.5;'>" + obj[i].Quantity + "</td>";
                                    sTable += "<td style='line-height:0.5;'>" + obj[i].PurchasePrice + "</td>";
                                    sTable += "</tr>";
                                    sCount = sCount + 1;
                                    $("#tblYarnDetailsList_body").append(sTable);
                                    var objAgent = new Object();
                                    objAgent.BarcodeID = obj[i].BarcodeID;
                                    objAgent.Barcodes = obj[i].Barcodes;
                                    objAgent.Quantity = obj[i].Quantity;
                                    objAgent.PurchasePrice = obj[i].PurchasePrice;
                                    gvalue.push(objAgent);
                                    $(".Yarnmodal-title").html("&nbsp;&nbsp; Product - " + obj[i].Product.ProductName + " | " + obj[i].Product.SMSCode + " | " + obj[i].Product.ProductCode);


                                }

                            }
                            else { $("#divYarnDetailsList").empty(); }
...
$("#divYarnDetailsList").on('change', "input[type='radio']", function (e) {
    var Balance = 0;
    $.each($("input[name='PurchaseName']:checked"), function () {
        ...
        for (var i = 0; i < gvalue.length; i++) {
            if (gvalue[i].BarcodeID == $(this).val()) {
                $("#hdnBarcodeID").val(gvalue[i].BarcodeID);
                $("#txtQuantity").val(gvalue[i].Quantity);
                $("#txtRate").val(gvalue[i].PurchasePrice).change();
                $("#txtBarcode").val(gvalue[i].Barcodes);
            }
                ...
                if (objResponse.Status == "Success") {
                    if (objResponse.Value != null && objResponse.Value != "NoRecord" && objResponse.Value != "Error") {
                        ...
                        if (obj != null) {
                            ...
                            $("#hdnSalesLimit").val(obj.Limit_SalesAmount);
                            $("#hdnSalesDiscountPercent").val(obj.Default_DiscountPercent);
...
function GetProductList(ddlname) {
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/VHMSService.svc/GetYarnProductList",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
        success: function (data) {
...
$("#btnAddMagazine,#btnUpdateMagazine").click(function () {
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/VHMSService.svc/GetBarcodeStockByID",
        data: JSON.stringify({ iStockID: $("#txtBarcode").val() }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
        success: function (data) {
    ...
    //GetProductDetails(ObjData.Product.ProductID, oProduct.SMSCode, 0, $("#ddlCustomerName").val(), $("#hdnSalesEntryID").val());
    ...
    if (gData.length > 0) {
        ...
        for (var i = 0; i < gData.length; i++) {
            if (gData[i].StatusFlag != "D") {
                ...
                //if (gData[i].SalesEntryID == 0)
                //    GetProductDetails(gData[i].Product.ProductID, 0, $("#ddlCustomerName").val(), 0);
                //else
                //    GetProductDetails(gData[i].Product.ProductID, gData[i].PurchaseID, $("#ddlCustomerName").val(), gData[i].SalesEntryID);
    ...
    for (var i = 0; i < data.length; i++) {
        if (data[i].sNO == ID) {
            if (data[i].StatusFlag != "D") {
                $("#txtSNo").val(ID);
                $("#hdnOPSNo").val(ID);
                $("#ddlProductName").attr("disabled", true);
...
function GetRecord() {
    ...
    $.ajax({
        ...
        data: JSON.stringify({ PublisherID: 0, IsRetail: 0, IsYarnBill: 1 }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            if (data.d != "") {
                        ...
                        if (obj.length > 0) {
                            ...
                            for (var index = 0; index < obj.length; index++) {
                                if (obj[index].IsActive == "1") {
                                    ...
                                    table += "<td style='text-align:center;'><a href='#' id=" + obj[index].SalesEntryID + " class='PrintSales' title='Click here to Print Yarn Invoice'></i><i class='fa fa-print text-green'/></a></td>";
                                    table += "<td style='text-align:center;'><a href='#' id=" + obj[index].SalesEntryID + " class='einvoice' invoice='" + obj[index].InvoiceNo + "' title='E-Invoice JSON format'><i class='fa fa-file-code-o text-black'></i></a></td>";
...
function GetSearchRecord(iDetails) {
    ...
    $.ajax({
        ...
        data: JSON.stringify({ ID: iDetails, IsRetail: 0, IsYarnBill: 1 }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
        success: function (data) {
...
function GetLastSalesEntryByID() {
    ...
    $.ajax({
        ...
        data: JSON.stringify({ ID: $("#hdnSalesID").val(), IsRetail: 0, IsYarnBill: 1 }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
        success: function (data) {
...
function EditRecord(id) {
    ...
    $.ajax({
        ...
        data: JSON.stringify({ ID: id, IsRetail: 0, IsYarnBill: 1}),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
        success: function (data) {
...
function EInvoice_Export(id, invoice) {
    ...
    $.ajax({
        type: "POST",
        //url: "WebServices/VHMSService.svc/GetSalesEntryJsonFormat",
        url: "WebServices/VHMSService.svc/GetSalesEntryJsonHSNFormat",
        data: JSON.stringify({ ID: id }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
