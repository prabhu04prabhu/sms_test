...
$("#btnAddNew").click(function () {
    ...
    $("#btnAddMagazine").show();
    $("#btnUpdateMagazine").hide();
    $("#chkIsDiscount").prop("checked", false).change();
...
$("#btnAddMagazine,#btnUpdateMagazine").click(function () {
    ...
    for (var i = 0; i < gOPBillingList.length; i++) {
        if (gOPBillingList[i].Product.ProductID == $("#ddlProductName").val() && gOPBillingList[i].StatusFlag != "D") {
    ...
    if ($("#hdnPurchaseReturnID").val() == 0) {
        ...
        $.ajax({
            ...
            data: JSON.stringify({ ID: $("#ddlProductName").val(), transaction_id: $("#hdnPurchaseReturnID").val(), type:"PR" }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
            success: function (data) {
                    ...
                    if (objResponse.Value != null && objResponse.Value != "NoRecord") {
                        ...
                        if (obj.length > 0) {
                            ...
                            for (var index = 0; index < obj.length; index++) {
                                ...
                                if (ActionUpdate == "1")
                                { table += "<td style='text-align:center;'><a href='#' id=" + obj[index].PurchaseReturnID + " class='Edit' title='Click here to Edit'><i class='fa fa-lg fa-edit'/></a></td>"; }
                                else { table += "<td></td>"; }
                                ...
                                table += "<td style='text-align:center;'><a href='#' id=" + obj[index].PurchaseReturnID + " class='einvoice' invoice='" + obj[index].ReturnNo + "_" + obj[index].Supplier.SupplierName + "' title='E-Invoice JSON format'><i class='fa fa-file-code-o text-black'></i></a></td>";
...
function EInvoice_Export(id, invoice) {
    ...
    $.ajax({
        ...
        url: "WebServices/VHMSService.svc/GetPurchaseReturnJsonHSNFormat",
        data: JSON.stringify({ ID: id }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
...
function CalculateAmount() {
    ...
    for (var i = 0; i < gOPBillingList.length; i++) {
        if (gOPBillingList[i].StatusFlag != "D") {
            ...
            if ($("#chkIsDiscount").prop('checked') == true)
                iGross = iGross + parseFloat(gOPBillingList[i].SubTotal)
            else
...
$("#btnSave,#btnUpdate").click(function () {
    ...
    ObjOPBilling.ACK_No = $("#txtACKNo").val();
    ObjOPBilling.IRN_No = $("#txtIRNNo").val();

    ObjOPBilling.IsDiscount = $("#chkIsDiscount").is(':checked') ? "1" : "0";
                ...
                if (objResponse.Status == "Success") {
                    if (objResponse.Value != null && objResponse.Value != "NoRecord" && objResponse.Value != "Error") {
                        ...
                        if (obj != null) {
                            ...
                            $("#chkIsDiscount").prop("checked", obj.IsDiscount ? true : false).change();
