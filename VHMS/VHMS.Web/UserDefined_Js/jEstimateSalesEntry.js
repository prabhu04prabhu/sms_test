            ...
            if (data.d != "") {
                ...
                if (objResponse.Status == "Success") {
                    if (objResponse.Value != null && objResponse.Value != "NoRecord" && objResponse.Value != "Error") {
                        var obj = jQuery.parseJSON(objResponse.Value);
                        if (obj != null) {
                            $("#hdnMaxsalesDiscount").val(obj.MaxEstimateSalesDiscount);
...
$("#btnAddNew").click(function () {
    ...
    $("#hdnEstimateSalesID").val("0");
    $("#hdnConvertBill").val("0");
    $("#btnAddMagazine").show();
    $("#btnUpdateMagazine").hide();
    $("#txtCustomer").val('');
    $("#txtMobileNo").val('');
...
function GetProductDetails(id, smscode, value, SID, EstimateSalesID) {
    dProgress(true);
    $.ajax({
        type: "POST",
        url: "WebServices/VHMSService.svc/GetNewProductDetails",
        data: JSON.stringify({ ID: id, code: smscode, type: value, SupplierID: SID, EstimateSalesEntryID: EstimateSalesID }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
        success: function (data) {
                        ...
                        if (obj != null) {
                            ...
                            if (obj.length > 0) {
                                ...
                                for (var i = 0; i < obj.length; i++) {
                                    sTable = "<tr><td style='line-height:0.5;' id='" + obj[i].PK_EstimateSalesEntryTransID + "'>" + sCount + "</td>";
...
function GetEstimateSalesProductDetails(id, smscode, value, SID, EstimateSalesID) {
    dProgress(true);
    $.ajax({
        type: "POST",
        url: "WebServices/VHMSService.svc/GetNewProductDetails",
...
function GetAllProductDetails(id, smscode, value, SID, EstimateSalesID) {
    dProgress(true);
    $.ajax({
        type: "POST",
        url: "WebServices/VHMSService.svc/GetNewProductDetails",
                ...
                if (objResponse.Status == "Success") {
                    if (objResponse.Value != null && objResponse.Value != "NoRecord" && objResponse.Value != "Error") {
                        ...
                        if (obj != null) {
                            ...
                            $("#txtAddress").val('');
                            $("#ddlShippingAddress").val(0).change();
                            $("#ddlShippingAddress").empty();
                            $("#txtArea").val('');
                        }
                        dProgress(false);
                ...
                if (objResponse.Status == "Success") {
                    if (objResponse.Value != null && objResponse.Value != "NoRecord" && objResponse.Value != "Error") {
                        ...
                        if (obj != null) {
                            ...
                            $("#hdnEstimateSalesLimit").val(obj.Limit_EstimateSalesAmount);
                            $("#hdnEstimateSalesDiscountPercent").val(obj.Default_DiscountPercent);
...
$("#btnAddMagazine,#btnUpdateMagazine").click(function () {
    ...
    //if ($("#hdnCustomerID").val() == "0" || $("#hdnCustomerID").val() == undefined || $("#hdnCustomerID").val() == null) {
    //    $.jGrowl("Please select Customer", { sticky: false, theme: 'warning', life: jGrowlLife });
    //    $("#divCustomer").addClass('has-error'); $("#hdnCustomerID").focus(); return false;
    //}
    ...
    GetProductDetails(ObjData.Product.ProductID, oProduct.SMSCode, 0, $("#hdnCustomerID").val(), $("#hdnEstimateSalesEntryID").val());
    ...
    if (gData.length > 0) {
        ...
        for (var i = 0; i < gData.length; i++) {
            if (gData[i].StatusFlag != "D") {
                ...
                //if (gData[i].EstimateSalesEntryID == 0)
                //    GetProductDetails(gData[i].Product.ProductID, 0, $("#hdnCustomerID").val(), 0);
                //else
                //    GetProductDetails(gData[i].Product.ProductID, gData[i].PurchaseID, $("#hdnCustomerID").val(), gData[i].EstimateSalesEntryID);
...
function GetRecord() {
    ...
    $.ajax({
        ...
        data: JSON.stringify({ PublisherID: 0, IsRetail: 0 }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            if (data.d != "") {
                        ...
                        if (obj.length > 0) {
                            ...
                            for (var index = 0; index < obj.length; index++) {
                                if (obj[index].IsActive == "1") {
                                    ...
                                    table += "<td style='text-align:center;'><a href='#' id=" + obj[index].EstimateSalesEntryID + " class='PrintEstimateSales' title='Click here to Print Invoice'></i><i class='fa fa-print text-green'/></a></td>";
                                    ...
                                    //table += "<td style='text-align:center;'><a href='#' id=" + obj[index].EstimateSalesEntryID + " class='LR' title='Click here for LR Entry'><i class='fa fa-truck text-black'></i></a></td>";
                            ...
                            $(".PrintEstimateSales").click(function () {
...
$("#btnSave,#btnUpdate").click(function () {
    ...
    //if ($("#hdnCustomerID").val() == "0" || $("#hdnCustomerID").val() == undefined || $("#hdnCustomerID").val() == null) {
    //    $.jGrowl("Please select Customer", { sticky: false, theme: 'warning', life: jGrowlLife });
    //    $("#divCustomerName").addClass('has-error'); $("#hdnCustomerID").focus(); return false;
    //}
    ...
    oCustomerType.CustomertypeID = $("#hdnCustomerTypeID").val();
    oCustomerType.CustomerTypeName = $("#txtCustomertype").val();
    ObjOPBilling.CustomerType = oCustomerType;
    ObjOPBilling.EstimateSalesExchangeTrans = [];
    ...
    ObjOPBilling.EstimateSalesPoints = 0;
    ...
    if ($("#hdnEstimateSalesID").val() > 0) {
        ObjOPBilling.EstimateSalesEntryID = $("#hdnEstimateSalesID").val();
        ...
        if ($("#ddlPaymentMode").val() == "Credit") {
            ...
            var Balance = parseFloat($("#txtNetAmount").val()) - parseFloat($("#hdnBalanceAmt").val());
            ObjOPBilling.BalanceAmount = parseFloat($("#hdnBalanceAmt").val()) + parseFloat(Balance);
        }
        else {
...
$("#btnConvert").click(function () {
    var id = $("#hdnConvertBill").val();
    if ($("#hdnConvertBill").val() == 'true') {
        if (confirm('This Bill already Converted do you want convert Again ?')) { ConvertBillRecord(); }
    }
    else
        ConvertBillRecord();
});
...
function ConvertBillRecord() {
    ...
    for (var i = 0; i < gOPBillingList.length; i++) {
        if (gOPBillingList[i].StatusFlag != "D") {
            ...
            var tstock_val = 0;
            for (var ai = 0; ai < gOPBillingList.length; ai++) {
                if (gOPBillingList[ai].StatusFlag != "D" && gOPBillingList[ai].Product.ProductID == gOPBillingList[i].Product.ProductID) {
                    tstock_val = tstock_val + gOPBillingList[ai].Quantity;
                }
            }

            if (StockValue < tstock_val) {
    ...
    oCustomerType.CustomerTypeName = $("#txtCustomertype").val();
    ObjOPBilling.CustomerType = oCustomerType;
    ObjOPBilling.SalesExchangeTrans = [];
    var ostate = new Object();
    ostate.StateID = $("#hdnstateID").val();
    ObjOPBilling.State = ostate;
    ...
    UpdateEstimateSalesStatus($("#hdnEstimateSalesID").val());
            ...
            if (data.d != "") {
                ...
                if (objResponse.Status == "Success") {
                    if (objResponse.Value > 0) {
                        ...
                        $("#secHeader").removeClass('hidden');
                        $("#divOPBilling").hide();
                        var myWindow = window.open("frmSalesEntry.aspx", "_self");
                    }
                }
                else if (objResponse.Status == "Error") {
                    if (objResponse.Value == "0") {
                ...
                if (objResponse.Status == "Success") {
                    if (objResponse.Value != null && objResponse.Value != "NoRecord" && objResponse.Value != "Error") {
                        ...
                        if (obj != null) {
                            ...
                            $("#hdnEstimateSalesID").val(obj.EstimateSalesEntryID)
                            ...
                            $("#txtArea").val(obj.Customer.Area).change();
                            ...
                            $("#hdnConvertBill").val(obj.SalesConverted);
