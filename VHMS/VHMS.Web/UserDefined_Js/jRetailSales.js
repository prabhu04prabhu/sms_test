...
$("#btnAddNew").click(function () {
    ...
    $("#chkExchange").prop("checked", false).change();
    ...
    $("#imagefile4").val("");
    $("#txtRate").val("0");
    $("#ddlGift").append('<option value="' + '0' + '">' + '--Select--' + '</option>');
    $("#ddlGift").val("0").change();
...
function ClearOPBillingTab() {
    ...
    $("#ddlBillStatus").val("Completed Bill").change();
    $("#divBank").hide();
    $("#txtOPDNo").val("");
    gOPBillingList = [];
    $("#btnSave").show();
...
$("#txtCode").blur(function () {
    ...
    if ($("#txtCode").val().length > 2) {
        ...
        if ($("#ddlProductName").val() > 0)
            GetRate();
        if ($("#hdnOPSNo").val() == null || $("#hdnOPSNo").val() == "") {
            $("#txtQuantity").val("1").change();
            $("#btnAddMagazine").click();
        }
    }
                ...
                if (objResponse.Status == "Success") {
                    if (objResponse.Value != null && objResponse.Value != "NoRecord") {
                        ...
                        if (obj.length > 0) {
                            ...
                            for (var index = 0; index < obj.length; index++) {
                                if (obj[index].IsActive)
                                    $(sControlName).append("<option value='" + obj[index].CustomerID + "'>" + obj[index].CustomerName + " - " + obj[index].MobileNo + "</option>");
                            }
                        }
                        dProgress(false);
                    }
...
$("#ddlExchangeProductName").change(function () {
    ...
    if ($("#ddlExchangeProductName").val() > 0) {
        var ProdID = $("#ddlExchangeProductName option:selected").text()
        var result = ProdID.split('-');
        for (var i = 0; i < gSalesReturnList.length; i++) {
            if (gSalesReturnList[i].Product.SMSCode == result[1]) {
                    ...
                    if (objResponse.Value != null && objResponse.Value != "NoRecord" && objResponse.Value != "Error") {
                        ...
                        if (obj != null) {
                            ...
                            for (var index = 0; index < ObjProduct.length; index++) {
                                ...
                                $("#ddlExchangeProductName").append("<option value='" + ObjProduct[index].Product.ProductID + "'>" + ObjProduct[index].Product.ProductName + "-" + ObjProduct[index].Product.SMSCode + "</option>");
...
$("#btnAddExchange,#btnUpdateExchange").click(function () {
    ...
    ObjData.SalesEntryID = 0;
    ObjData.ExchangeSalesEntryID = $("#hdnSalesID").val();
    ObjData.SalesInvoiceNo = $("#txtExchangeBillNo").val();
    var oProduct = new Object();
...
function DisplayExchangeList(gData) {
    ...
    if (gData.length > 0) {
        ...
        for (var i = 0; i < gData.length; i++) {
            if (gData[i].StatusFlag != "D") {
                sTable = "<tr><td id='" + gData[i].SNo + "'>" + sCount + "</td>";
                sTable += "<td>" + gData[i].SalesInvoiceNo + "</td>";
                sTable += "<td>" + gData[i].Product.ProductName + "</td>";
                sTable += "<td>" + gData[i].Quantity + "</td>";
                sTable += "<td>" + gData[i].Rate + "</td>";
...
function Bind_ExchangeByID(ID, data) {
    ...
    for (var i = 0; i < data.length; i++) {
        if (data[i].sNO == ID) {
            ...
            $("#hdnSalesID").val(data[i].ExchangeSalesEntryID);
...
function Update_Exchange(oData) {
    for (var i = 0; i < gExchangeList.length; i++) {
        if (gExchangeList[i].sNO == oData.sNO) {
            ...
            gExchangeList[i].ExchangeSalesEntryID = oData.ExchangeSalesEntryID;
            gExchangeList[i].SalesInvoiceNo = oData.SalesInvoiceNo;
        }
    }
    DisplayExchangeList(gExchangeList);
    $("#btnAddExchange").show();
...
function GetProductDetails(id, value, SID, iCode) {
    dProgress(true);
    $.ajax({
        type: "POST",
        url: "WebServices/VHMSService.svc/GetProductDetailsBySMSCode",
        data: JSON.stringify({ ID: id, type: value, SupplierID: SID, iSMSCode: iCode }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
        success: function (data) {
...
function GetRecord() {
    ...
    $.ajax({
        type: "POST",
        url: "WebServices/VHMSService.svc/GetTopRetailsSalesEntry",
        data: JSON.stringify({ PublisherID: 0, IsRetail: 1, IsYarnBill: 0, iCustomerID: $("#ddlCustomer").val() }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            if (data.d != "") {
...
function GetLastSalesEntryByID() {
    ...
    $.ajax({
        ...
        data: JSON.stringify({ ID: $("#hdnRetailSalesID").val(), IsRetail: 1, IsYarnBill: 0 }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
        success: function (data) {
...
$("#btnSave,#btnUpdate").click(function () {
    ...
    if (gOPBillingList.length <= 0 && gExchangeList.length <= 0) {
    ...
    var iExchnageAmount = 0;
    for (var i = 0; i < gExchangeList.length; i++) {
        if (gExchangeList[i].StatusFlag != "D")
            iExchnageAmount = iExchnageAmount + 1;
    }

    if (iOPBillingAmount <= 0 && iExchnageAmount <= 0) {
        $.jGrowl("No Product has been added", { sticky: false, theme: 'warning', life: jGrowlLife });
    ...
    if ($("#hdnRetailSalesID").val() > 0) {
        ObjOPBilling.SalesEntryID = $("#hdnRetailSalesID").val();
                ...
                if (objResponse.Status == "Success") {
                    if (objResponse.Value != null && objResponse.Value != "NoRecord" && objResponse.Value != "Error") {
                        ...
                        if (obj != null) {
                            ...
                            $("#hdnRetailSalesID").val(obj.SalesEntryID);
                            ...
                            $("#txtNetAmount").val(obj.NetAmount).change();
                            ...
                            if (obj.ExchangeAmount > 0) {
                                $("#chkExchange").prop("checked", true).change();
                            ...
                            for (var index = 0; index < ObjProduct.length; index++) {
                                ...
                                objTemp.SalesEntryTransID = ObjProduct[index].SalesEntryTransID;
                                objTemp.SalesEntryID = ObjProduct[index].SalesEntryID;
                                objTemp.ExchangeSalesEntryID = ObjProduct[index].ExchangeSalesEntryID;
                                objTemp.SalesInvoiceNo = ObjProduct[index].SalesInvoiceNo;
                                objTemp.ProductID = ObjProduct[index].Product.ProductID;
                                objTemp.ProductName = ObjProduct[index].Product.ProductName;
                                objTemp.SMSCode = ObjProduct[index].Product.SMSCode;
                                objTemp.Quantity = ObjProduct[index].Quantity;
