<%@ Page Title="Work" Language="C#" MasterPageFile="~/VHMSMasterPage.master" AutoEventWireup="true" CodeFile="frmWork.aspx.cs" Inherits="frmWork" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="VHMSWebContent" runat="Server">
                        ...
                        <div class="modal-body">
                            <div class="row">
                                <div class="form-group col-md-8" id="divName">
                                    ...
                                    <input type="text" class="form-control" id="txtName" placeholder="Please enter Work Name"
    ...
    <script type="text/javascript">
        ...
        $("#btnAddNew").click(function () {
            ...
            $("#btnSave").show();
            $("#btnUpdate").hide();
            $(".modal-title").html("<i class='fa fa-plus-square'></i>&nbsp;&nbsp;Add Work");
        ...
        $("#btnSave,#btnUpdate").click(function () {
            ...
            if ($("#txtName").val().trim() == "" || $("#txtName").val().trim() == undefined) {
                $.jGrowl("Please enter Work Name", { sticky: false, theme: 'warning', life: jGrowlLife });
            ...
            var sMethodName;
            if ($("#hdnID").val() > 0) {
                Obj.WorkID = $("#hdnID").val();
                sMethodName = "UpdateWork";
            }
            else { sMethodName = "AddWork"; }

            SaveandUpdateWork(Obj, sMethodName);
                            ...
                            if (objResponse.Value != null && objResponse.Value != "NoRecord") {
                                ...
                                if (obj.length > 0) {
                                    ...
                                    for (var index = 0; index < obj.length; index++) {
                                        ...
                                        var table = "<tr id='" + obj[index].WorkID + "'>";
                                        table += "<td>" + (index + 1) + "</td>";
                                        table += "<td>" + obj[index].WorkName + "</td>";
                                        table += "<td>" + obj[index].Amount + "</td>";
                                        table += "<td class='hidden-xs'>" + TypeStatus + "</td>";

                                        if (ActionView == "1")
                                        { table += "<td style='text-align:center;'><a href='#' id=" + obj[index].WorkID + " class='View' title='Click here to View'><i class='fa fa-lg fa-clone text-yellow'/></a></td>"; }
                                        else { table += "<td></td>"; }

                                        if (ActionUpdate == "1")
                                        { table += "<td style='text-align:center;'><a href='#' id=" + obj[index].WorkID + " class='Edit' title='Click here to Edit'><i class='fa fa-lg fa-edit'/></a></td>"; }
                                        else
                                        { table += "<td></td>"; }

                                        if (ActionDelete == "1")
                                        { table += "<td style='text-align:center;'><a href='#' id=" + obj[index].WorkID + " class='Delete' title='Click here to Delete'><i class='fa fa-lg fa-trash-o text-red'/></a></td>"; }
                                        else
                                        { table += "<td></td>"; }
                                    ...
                                    $(".View").click(function () {
                                        if (ActionView == "1") {
                                            ...
                                            $(".modal-title").html("<i class='fa fa-clone'></i>&nbsp;&nbsp;View Work");
                                            $("#btnSave").hide();
                                            $("#btnUpdate").hide();
                                        }
                                        else {
        ...
        function SaveandUpdateWork(Obj, sMethodName) {
            $.ajax({
                type: "POST",
                ...
                success: function (data) {
                    if (data.d != "") {
                        ...
                        if (objResponse.Status == "Success") {
                            ...
                            else if (objResponse.Value == "Work_A_01" || objResponse.Value == "Work_U_01") {
        ...
        function EditRecord(id) {
            ...
            $.ajax({
                type: "POST",
                url: "WebServices/VHMSService.svc/GetWorkByID",
                        ...
                        if (objResponse.Status == "Success") {
                            if (objResponse.Value != null && objResponse.Value != "NoRecord" && objResponse.Value != "Error") {
                                ...
                                if (obj != null) {
                                    ...
                                    $("#hdnID").val(obj.WorkID);
                                    $("#txtName").val(obj.WorkName);
                                    $("#txtAmount").val(obj.Amount);

                                    $("#chkStatus").prop("checked", obj.IsActive ? true : false);

                                    $('#compose-modal').modal({ show: true, backdrop: true });
                                    $(".modal-title").html("<i class='fa fa-pencil'></i>&nbsp;&nbsp;Edit Work");
                                }
                                dProgress(false);
                            }
                            else if (objResponse.Value == "NoRecord") {
        ...
        function DeleteRecord(id) {
            $.ajax({
                type: "POST",
                url: "WebServices/VHMSService.svc/DeleteWork",
                data: JSON.stringify({ ID: id }),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                async: false,
                success: function (data) {
                    if (data.d != "") {
                        ...
                        if (objResponse.Status == "Success") {
                            ...
                            else if (objResponse.Value == "Work_R_01" || objResponse.Value == "Work_D_01") {
