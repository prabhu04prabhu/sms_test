...
<asp:Content ID="Content2" ContentPlaceHolderID="VHMSWebContent" runat="Server">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="row" style="display:none;">
                                <div class="form-group col-md-2">
                                    <button type="submit" class="btn btn-info pull-left" style="background-color: darkgreen;" id="btncalculateTDS" tabindex="-1">
                                        <i class="fa fa-calculator"></i>&nbsp;&nbsp;
                                Calculate TDS</button>
                                </div>
                                <div class="form-group col-md-3" id="divTotalValue">
                                    <label>
                                        Total Value</label>
                                    <input id="txtTotalValue" class="form-control decimal" maxlength="255" tabindex="-1" readonly autocomplete="off" />
                                </div>
                                <div class="form-group col-md-2" id="divTDSPercent">
                                    <label>
                                        TDS %</label>
                                    <input id="txtTDSPercent" class="form-control decimal" maxlength="255" tabindex="-1" value="0.1" autocomplete="off" />
                                </div>
                                <div class="form-group col-md-3" id="divTDSAmt">
                                    <label>
                                        TDS Amt</label>
                                    <input id="txtTDSAmt" class="form-control decimal" maxlength="255" tabindex="-1" autocomplete="off" />
                                </div>
                            </div>
    ...
    <script type="text/javascript">
        ...
        //$("#btncalculateTDS").click(function () {
        //    var Balance = 0;
        //    $.each($("input[name='PurchaseName']:checked"), function () {

        //        for (var i = 0; i < gvalue.length; i++) {
        //            if (gvalue[i].PurchaseID == $(this).val()) {
        //                Balance = parseFloat(Balance) + gvalue[i].NetAmount - gvalue[i].ReturnAmount - gvalue[i].PurchaseDiscount;
        //            }
        //        }
        //    });

        //    $("#txtTotalValue").val(parseFloat(Balance).toFixed(2)).change();

        //    return false;
        //});

        //$("#txtTotalValue,#txtTDSPercent").change(function () {
        //    CalculateAmount();
        //});

        //function CalculateAmount() {
        //    var iTotalValue = 0, iTDSPercent = 0, iTDSAmt = 0, iAmount = 0, iPaidAmt = 0;

        //    var iTotalValue = parseFloat($("#txtTotalValue").val());
        //    if (isNaN(iTotalValue)) iTotalValue = 0;

        //    var iTDSPercent = parseFloat($("#txtTDSPercent").val());
        //    if (isNaN(iTDSPercent)) iTDSPercent = 0;

        //    iTDSAmt = (parseFloat(iTotalValue) * parseFloat(iTDSPercent) / 100);
        //    if (isNaN(iPaidAmt)) iTDSAmt = 0;

        //    $("#txtTDSAmt").val(parseFloat(iTDSAmt).toFixed(2));

        //    var iAmount = parseFloat($("#txtAmount").val());
        //    if (isNaN(iAmount)) iAmount = 0;

        //    var iPaidAmt = parseFloat(iAmount - iTDSAmt)
        //    iTDSAmt = (parseFloat(iAmount) - parseFloat(iTDSAmt));
        //    if (isNaN(iPaidAmt)) iTDSAmt = 0;

        //    $("#txtAmount").val(parseFloat(iPaidAmt).toFixed(2)).change();
        //}
        ...
        $("#ddlSupplier").change(function () {
            ...
            if ($("#ddlSupplier").val() > 0) {
                ...
                GetOnAccountAmount($("#ddlSupplier").val());
                GetLastTransaction();
                GetPendingBill();
            }
        });
        ...
        function GetPendingBill() {
            ...
            $.ajax({
                ...
                data: JSON.stringify({ PublisherID: $("#ddlSupplier").val(), BillType: 1 }),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (data) {
                    if (data.d != "") {
        ...
        function DisplayPendingBillList(gData) {
            ...
            if (gData.length > 0) {
                ...
                sTable += "<th class='" + sColorCode + "'>TDS Amt</th>";
                sTable += "<th class='" + sColorCode + "'>Pur. Disc</th>";
                sTable += "<th class='" + sColorCode + "'>Paid Amt</th>";
                sTable += "<th class='" + sColorCode + "'>Balance Amt</th>";
                sTable += "<th class='" + sColorCode + "'>Payment Disc.</th>";
                sTable += "<th class='" + sColorCode + "'>Courier/Other Charges</th>";
                ...
                for (var i = 0; i < gData.length; i++) {
                    ...
                    sTable += "<td>" + gData[i].PurchaseDiscount + "</td>";
                    ...
                    objAgent.ReturnAmount = gData[i].ReturnAmount;
                    objAgent.PurchaseDiscount = gData[i].PurchaseDiscount;
                    objAgent.NetAmount = gData[i].NetAmount;
                    objAgent.PaymentDiscount = payamt;
                    objAgent.OtherDiscount = otCharges;
