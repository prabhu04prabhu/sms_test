<%@ Page Title="Expense Payment" Language="C#" MasterPageFile="~/VHMSMasterPage.master" AutoEventWireup="true" CodeFile="frmExpensePayment.aspx.cs" Inherits="frmExpensePayment" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="VHMSWebContent" runat="Server">
    <div class="container-wrapper hidden">
        <section class="content-header">
            ...
            <div class="form-group  col-md-4" style="margin-left: 255px; margin-top: -34px;">
                <label>
                    Party Name</label>
                <select id="ddlPartyName" class="form-control select2" data-placeholder="Select Category Name" tabindex="-1"></select>
            </div>
            <small></small>
                                ...
                                <table id="tblRecord" class="table table-striped table-bordered bg-info" width="100%">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Ref No</th>
                                            <th>Entry Date</th>
                                            <th class="hidden-xs">Party</th>
                                            <th class="hidden-xs">Bill Nos</th>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-md-3" id="divSupplier">
                                    ...
                                    <select id="ddlSupplier" class="form-control select2" data-placeholder="Select Party" tabindex="3">
                                    </select>
                                </div>
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-md-2" id="divAdvance">
                                    <input type="checkbox" id="chkAdvance" style="margin: 30px 0 0 0; width: 18px; height: 18px;" tabindex="-1" /><label> Advance</label>
                                </div>
    ...
    <script type="text/javascript">
        ...
        //        var myWindow = window.open("frmDExpensePayment.aspx", "_self");
        //    }
        //};
        var gvalue = []; var iAmount = 0; var iCount = 0;

        var pageUrl = '<%=ResolveUrl("~/frmExpensePayment.aspx") %>';
        $(document).ready(function () {
            ...
            GetSupplierListBySearch("ddlPartyName");
        ...
        function DocumentuploadComplete(sender, args) {
            $.ajax({
                ...
                dataType: "json",
                success: function (r) {
                    $("#hdnImgupload1").val("./images/Documents/ExpensePayment/" + r.d);
                    $("#imgUpload1_view").val("/images/Documents/ExpensePayment/" + r.d);
                    // $get("imgUpload1").src = "./images/Documents/ExpensePayment/" + r.d;
                },
        ...
        $("#divPendingBillList").on('change', "input[type='checkbox']", function (e) {
            ...
            $.each($("input[name='PurchaseName']:checked"), function () {
                for (var i = 0; i < gvalue.length; i++) {
                    if (gvalue[i].ExpenseID == $(this).val()) {
            ...
            $.each($("input[name='DiscountName']:checked"), function () {
                for (var i = 0; i < gvalue.length; i++) {
                    if (gvalue[i].PurchaseID == $(this).val()) {
                        iDiscount = parseFloat(iDiscount) + parseFloat(gvalue[i].ExpensePaymentDiscount);
        ...
        $("#btnAddNew").click(function () {
            ...
            $(".modal-title").html("<i class='fa fa-plus-square'></i>&nbsp;&nbsp;Add ExpensePayment");
        ...
        $("#btnSave,#btnUpdate,#btnPrint").click(function () {
            ...
            if ($("#ddlPaymentMode").val() == "0" || $("#ddlPaymentMode").val() == undefined || $("#ddlPaymentMode").val() == null) {
                $.jGrowl("Please select ExpensePayment Mode", { sticky: false, theme: 'warning', life: jGrowlLife });
            ...
            if ($("#ddlPaymentMode").val() != 2) {
                if ($("#txtCollectionDate").val().trim() == "" || $("#txtCollectionDate").val().trim() == undefined) {
                    $.jGrowl("Please select ExpensePayment Date", { sticky: false, theme: 'warning', life: jGrowlLife });
            ...
            $.each($("input[name='PurchaseName']:checked"), function () {
                ...
                for (var i = 0; i < gvalue.length; i++) {
                    if (gvalue[i].ExpenseID == $(this).val()) {
                        Balance = parseFloat(parseFloat(Balance) + gvalue[i].BalanceAmount).toFixed(2);
                        iCount = 1
                    }
                }
            });
            if (gvalue.length >= 1) {
                if (!$("#chkAdvance").is(':checked')) {
                    if (parseFloat($("#txtBalanceAmount").val()) + parseFloat($("#txtOnAccount").val()) + parseFloat($("#hdnAmount").val()) < (parseFloat($("#txtAmount").val()) + parseFloat($("#txtDiscountAmount").val()) + parseFloat($("#txtOtherDiscount").val()))) {
                ...
                var id = $("#txtBalanceAmount").val();
                var id1 = $("#hdnAmount").val();
                var id2 = $("#txtAmount").val();
                var id4 = $("#txtDiscountAmount").val();
                var id5 = $("#txtOtherDiscount").val();
            ...
            Obj.IsAdvance = $("#chkAdvance").is(':checked') ? "1" : "0";

            var sMethodName;
            if ($("#hdnID").val() > 0) {
                Obj.ExpensePaymentID = $("#hdnID").val();
                sMethodName = "UpdateExpensePayment";
            }
            else { sMethodName = "AddExpensePayment"; }

            SaveandUpdateExpensePayment(Obj, sMethodName);

            if (this.id == "btnPrint") {
                SetSessionValue("PrintExpensePaymentID", $("#hdnID").val());
                var myWindow = window.open("PrintChequeExpenese.aspx", "MsgWindow");
            }
            else {
                $("#hdnID").val(0);
            }
        ...
        $("#ddlSupplier").change(function () {
            ...
            if ($("#ddlSupplier").val() > 0) {
                ...
                GetBalanceAmount($("#ddlSupplier").val());
                GetLastTransaction();
                GetPendingBill();
            }
        });
        ...
        function GetBalanceAmount(id) {
            ...
            $.ajax({
                type: "POST",
                url: "WebServices/VHMSService.svc/GetExpenseBalanceAmount",
                    ...
                    if (data.d != "") {
                        ...
                        if (objResponse.Status == "Success") {
                            if (objResponse.Value != null && objResponse.Value != "NoRecord" && objResponse.Value != "Error") {
                                var obj = jQuery.parseJSON(objResponse.Value);
                                if (obj != null) {
                                    $("#txtBalanceAmount").val(obj.OnAccount);
                                }
                                dProgress(false);
                            }
                            else if (objResponse.Value == "NoRecord") {
        ...
        $("#txtDiscountAmount,#txtAmount,#txtOtherDiscount, #chkAddOnAccount").change(function () {
            ...
            if ($("#chkAddOnAccount").prop('checked') == false)
                iOnAccount = 0;
            $("#txtTotalAmount").val((parseFloat(iDiscount) + parseFloat(iAmount) + parseFloat(iOnAccount) + parseFloat(iOther)).toFixed(2));
            if (iCount == 1)
                CalculateTrans();
            //else
            //    CalculateBalanceTrans();
        ...
        function GetLastTransaction() {
            ...
            $.ajax({
                type: "POST",
                url: "WebServices/VHMSService.svc/GetLastExpensePaymentDetails",
                data: JSON.stringify({ ID: $("#ddlSupplier").val(), Type: 1 }),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (data) {
                    if (data.d != "") {
        ...
        function GetPendingBill() {
            ...
            $.ajax({
                type: "POST",
                url: "WebServices/VHMSService.svc/GetPendingExpense",
                    ...
                    if (data.d != "") {
                        ...
                        if (objResponse.Status == "Success") {
                            ...
                            if (objResponse.Value != null && objResponse.Value != "NoRecord") {
                                var obj = $.parseJSON(objResponse.Value);
                                if (obj.length > 0) {
                                    $("#txtOnAccount").val(obj[0].OnAmount);
        ...
        function DisplayOPBillingList(gData) {
            ...
            if (gData.length > 0) {
                ...
                sTable += "<th style='line-height:0.5;' class='" + sColorCode + "'>Expense No</th>";
                ...
                sTable += "<th style='line-height:0.5;' class='" + sColorCode + "'>ExpensePayment Date</th>";
                ...
                for (var i = 0; i < gData.length; i++) {
                    ...
                    sTable += "<td style='line-height:0.5;'>" + gData[i].ExpenseNo + "</td>";
        ...
        function DisplayPendingBillList(gData) {
            ...
            if (gData.length > 0) {
                ...
                sTable += "<th class='" + sColorCode + "'>Bill Date</th>";
                sTable += "<th class='" + sColorCode + "'>Expense No</th>";
                sTable += "<th class='" + sColorCode + "'>Taxable Amount</th>";
                ...
                for (var i = 0; i < gData.length; i++) {
                    ...
                    if ($("#hdnID").val() > 0) {
                        sTable += "<td><label><input id='chk_" + gData[i].ExpenseID + "' type='checkbox' value='" + gData[i].ExpenseID + "'  name='PurchaseName' disabled='disabled'/>&nbsp;" + gData[i].BillNo + "</label></td>";
                    }
                    else {
                        sTable += "<td><label><input id='chk_" + gData[i].ExpenseID + "' type='checkbox' value='" + gData[i].ExpenseID + "'  name='PurchaseName'/>&nbsp;" + gData[i].BillNo + "</label></td>";
                    }
                    // sTable += "<td>" + gData[i].ExpenseNo + "</td>";
                    sTable += "<td>" + gData[i].sBillDate + "</td>";
                    sTable += "<td>" + gData[i].ExpenseNo + "</td>";
                    sTable += "<td>" + gData[i].TotalAmount + "</td>";
                    ...
                    //if ($("#hdnID").val() > 0) {
                    //    sTable += "<td><label><input id='chkdiscount_" + gData[i].PurchaseID + "' type='checkbox' value='" + gData[i].PurchaseID + "'  name='DiscountName' disabled='disabled'/>&nbsp;" + gData[i].ExpensePaymentDiscount + "</label></td>";
                    //} else {
                    //    sTable += "<td><label><input id='chkdiscount_" + gData[i].PurchaseID + "' type='checkbox' value='" + gData[i].PurchaseID + "'  name='DiscountName'/>&nbsp;" + gData[i].ExpensePaymentDiscount + "</label></td>";
                    //}
                    ...
                    objAgent.ExpenseID = gData[i].ExpenseID;
        ...
        $("#ddlPaymentMode").change(function () {
            ...
            if (iPaymentMode != undefined && iPaymentMode > 0) {
                ...
                if (iPaymentMode == 2) {
                    ...
                    $("#lblCollection").text("ExpensePayment Date");
                    ...
                    if (data.d != "") {
                        ...
                        if (objResponse.Status == "Success") {
                            if (objResponse.Value != null && objResponse.Value != "NoRecord" && objResponse.Value != "Error") {
                                var obj = jQuery.parseJSON(objResponse.Value);
                                if (obj != null) {
                                    $("#ddlPaymentMode").val(obj.ExpensePaymentModeID).change();
                                    $("#ddlBank").val(obj.ExpenseBank.LedgerID).change();
                                }
                                dProgress(false);
                            }
                            else if (objResponse.Value == "NoRecord") {
        ...
        function GetSupplierListBySearch(ddlname) {
        ...
        function SaveandUpdateExpensePayment(Obj, sMethodName) {
            $.ajax({
                type: "POST",
                    ...
                    if (data.d != "") {
                        ...
                        if (objResponse.Status == "Success") {
                            if (objResponse.Value > 0) {
                                ...
                                if (sMethodName == "AddExpensePayment") {
                                    $.jGrowl("Added Successfully", { sticky: false, theme: 'success', life: jGrowlLife });

                                }
                                else if (sMethodName == "UpdateExpensePayment") { $.jGrowl("Updated Successfully", { sticky: false, theme: 'success', life: jGrowlLife }); }
                            ...
                            else if (objResponse.Value == "ExpensePayment_A_01" || objResponse.Value == "ExpensePayment_U_01") {
        ...
        function GetRecord() {
            ...
            $.ajax({
                type: "POST",
                url: "WebServices/VHMSService.svc/GetExpensePayment",
                data: JSON.stringify({ Type: 1, iPartyID: $("#ddlPartyName").val() }),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (data) {
                    if (data.d != "") {
                            ...
                            if (objResponse.Value != null && objResponse.Value != "NoRecord") {
                                ...
                                if (obj.length > 0) {
                                    ...
                                    for (var index = 0; index < obj.length; index++) {
                                        ...
                                        var table = "<tr id='" + obj[index].ExpensePaymentID + "'>";
                                        table += "<td>" + (index + 1) + "</td>";
                                        table += "<td>" + obj[index].VoucherNo + "</td>";
                                        table += "<td>" + obj[index].sVoucherDate + "</td>";
                                        table += "<td class='hidden-xs'>" + obj[index].Supplier.LedgerName + "</td>";
                                        ...
                                        if (ActionView == "1") { table += "<td><a href='#' id=" + obj[index].ExpensePaymentID + " class='View' title='Click here to View'><i class='fa fa-lg fa-clone text-yellow'/></a></td>"; }
                                        else { table += "<td></td>"; }

                                        if (ActionUpdate == "1") { table += "<td style='text-align:center;'><a href='#' id=" + obj[index].ExpensePaymentID + " class='Edit' title='Click here to Edit'><i class='fa fa-lg fa-edit'/></a></td>"; }
                                        else { table += "<td></td>"; }

                                        if (ActionDelete == "1") { table += "<td ><a href='#' id=" + obj[index].ExpensePaymentID + " class='Delete' title='Click here to Delete'><i class='fa fa-lg fa-trash-o text-red'/></a></td>"; }
                                        else { table += "<td></td>"; }

                                        table += "<td style='text-align:center;'><a href='#' id=" + obj[index].ExpensePaymentID + " class='Print' title='Click here to Print ExpensePayment'></i><i class='fa fa-print text-green'/></a></td>";

                                        //if (obj[index].PaymentModeID == 2) {
                                        //    table += "<td style='text-align:center;'><a href='#' id=" + obj[index].ExpensePaymentID + " class='PrintCheque' title='Click here to Print Cheque ExpensePayment'></i><i class='fa fa-print text-green'/></a></td>";
                                        //}
                                        //else{
                                        //table += "<td></td>";
                                    ...
                                    $(".View").click(function () {
                                        if (ActionView == "1") {
                                            ...
                                            $(".modal-title").html("<i class='fa fa-clone'></i>&nbsp;&nbsp;View ExpensePayment");
                                            $("#btnSave").hide();
                                            $("#btnUpdate").hide();
                                            $("#btnPrint").hide();
                                            $("#btnPrintbill").show();
                                    ...
                                    $(".Edit").click(function () {
                                        if (ActionView == "1") {
                                            EditRecord($(this).parent().parent()[0].id);
                                            $(".modal-title").html("<i class='fa fa-clone'></i>&nbsp;&nbsp;Edit ExpensePayment");
                                            $("#btnSave").hide();
                                            $("#btnUpdate").show();
                                            $("#btnPrint").hide();
                                    ...
                                    $(".Print").click(function () {
                                        SetSessionValue("PrintExpensePaymentID", parseInt($(this).parent().parent()[0].id));
                                        var myWindow = window.open("PrintChequeExpenese.aspx", "MsgWindow");
                                    });
                                    $(".PrintCheque").click(function () {
                                        SetSessionValue("PrintExpensePaymentID", parseInt($(this).parent().parent()[0].id));
                                        var myWindow = window.open("PrintChequeExpensePayment.aspx", "MsgWindow");
                                    });
        ...
        function EditRecord(id) {
            ...
            $.ajax({
                type: "POST",
                url: "WebServices/VHMSService.svc/GetExpensePaymentByID",
                        ...
                        if (objResponse.Status == "Success") {
                            if (objResponse.Value != null && objResponse.Value != "NoRecord" && objResponse.Value != "Error") {
                                ...
                                if (obj != null) {
                                    ...
                                    $("#hdnID").val(obj.ExpensePaymentID);
                                    $("#txtVoucherNo").val(obj.VoucherNo);
                                    $("#txtVoucherDate").val(obj.sVoucherDate);
                                    $("#ddlSupplier").val(obj.Supplier.LedgerID).change();
                                    ...
                                    $("[id*=imgUpload1]").attr("src", obj.DocumentPath);
                                    $("#chkAdvance").prop("checked", obj.IsAdvance ? true : false);
                                    ...
                                    $('#compose-modal').modal({ show: true, backdrop: true });
                                    $(".modal-title").html("<i class='fa fa-pencil'></i>&nbsp;&nbsp;Edit ExpensePayment");
                                }
                                dProgress(false);
                            }
                            else if (objResponse.Value == "NoRecord") {
        ...
        function DeleteRecord(id) {
            $.ajax({
                type: "POST",
                url: "WebServices/VHMSService.svc/DeleteExpensePayment",
                data: JSON.stringify({ ID: id }),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                async: false,
                success: function (data) {
                    if (data.d != "") {
                        ...
                        if (objResponse.Status == "Success") {
                            ...
                            else if (objResponse.Value == "ExpensePayment_R_01" || objResponse.Value == "ExpensePayment_D_01") {
